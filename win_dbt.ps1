# ==============================================================================
# Windows Debloat Tool (Win10 + Win11 combined)
#
# HOW TO RUN:
...
$host.UI.RawUI.WindowTitle = "Windows Debloat Tool"
...
function Remove-AppxSafe($name) {
    ...
    if ($prov) { $prov | Remove-AppxProvisionedPackage -Online | Out-Null }
}
...
function Ensure-Winget {
    ...
    try {
        ...
        Invoke-WebRequest -Uri $xamlUrl -OutFile "$tempDir\xaml.appx"
        ...
        if (Get-Command winget -ErrorAction SilentlyContinue) {
            ...
            Write-Host " [!] winget install may need a session restart to appear on PATH." -ForegroundColor Magenta
        }
    } catch {
        Write-Host " [!] Automatic winget install failed. Install manually from the Microsoft Store (App Installer)." -ForegroundColor Red
    }
}

# ==============================================================================
# REMOVE EDGE (shared by both OS)
# ==============================================================================
...
function Invoke-RemoveEdge {
    ...
    foreach ($task in $edgeTasks) { Disable-ScheduledTask -TaskName $task -ErrorAction SilentlyContinue | Out-Null }

    Write-Step "Disabling Edge Update services..."
    Stop-Service "edgeupdate" -Force; Set-Service "edgeupdate" -StartupType Disabled
    Stop-Service "edgeupdatem" -Force; Set-Service "edgeupdatem" -StartupType Disabled
...
function Invoke-Debloat10 {

    Write-Section "REMOVING BLOATWARE APPS"
    $appsToRemove = @(
        "Microsoft.3DBuilder","Microsoft.3DPrint","Microsoft.BingFinance","Microsoft.BingFoodAndDrink",
        "Microsoft.BingHealthAndFitness","Microsoft.BingMaps","Microsoft.BingNews","Microsoft.BingSports",
        "Microsoft.BingTranslator","Microsoft.BingTravel","Microsoft.BingWeather","Microsoft.GetHelp",
        ...
        "Microsoft.Office.Sway","Microsoft.OneConnect","Microsoft.OutlookForWindows","Microsoft.People",
        "Microsoft.PowerAutomateDesktop","Microsoft.Print3D","Microsoft.SkypeApp","Microsoft.StorePurchaseApp",
        "Microsoft.Todos","Microsoft.Wallet","Microsoft.WebMediaExtensions","Microsoft.WebpImageExtension",
        "Microsoft.WindowsFeedbackHub","Microsoft.WindowsMaps","Microsoft.WindowsPhone","Microsoft.WindowsReadingList",
        "Microsoft.WindowsSoundRecorder","Microsoft.YourPhone","Microsoft.ZuneVideo",
        "Microsoft.GamingApp","Microsoft.XboxApp","Microsoft.XboxGameCallableUI","Microsoft.XboxGameOverlay",
        "Microsoft.XboxGamingOverlay","Microsoft.XboxIdentityProvider","Microsoft.XboxSpeechToTextOverlay","Microsoft.Xbox.TCUI",
        "MicrosoftTeams","MSTeams","Microsoft.Teams","Microsoft.MicrosoftTeams",
        "Microsoft.549981C3F5F10",
        "ACGMediaPlayer","ActiproSoftwareLLC","AdobeSystemsIncorporated.AdobePhotoshopExpress","Amazon.com.Amazon",
        "AmazonVideo.PrimeVideo","Clipchamp.Clipchamp","Disney.37853D22215E2","Dolby","Duolingo-LearnLanguagesforFree",
        "EclipseManager","Facebook","Flipboard","HULULLC.HULUPLUS","king.com.BubbleWitch3Saga","king.com.CandyCrushFriends",
        "king.com.CandyCrushSaga","king.com.CandyCrushSodaSaga","LinkedInforWindows","MarchofEmpires","Netflix",
        "PandoraMediaInc","Plex","PricelinePartnerNetwork","Shazam","Spotify","SpotifyAB.SpotifyMusic","TheNewYorkTimes",
        "TikTok","TuneIn","Twitter","Wunderlist","Microsoft.Holographic.FirstRun"
    )
    foreach ($app in $appsToRemove) { Remove-AppxSafe $app }

    Write-Section "DISABLING CORTANA"
    Write-Step "Disabling Cortana via Group Policy..."
    ...
    Write-Step "Preventing OneDrive from being reinstalled via Group Policy..."
    $odPolicy = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\OneDrive"
    If (!(Test-Path $odPolicy)) { New-Item -Path $odPolicy -Force | Out-Null }
    Set-ItemProperty -Path $odPolicy -Name "DisableFileSyncNGSC" -Value 1 -Type DWord
    ...
    Stop-Service "WMPNetworkSvc" -Force; Set-Service "WMPNetworkSvc" -StartupType Disabled
    ...
    Write-Step "Disabling Consumer Features (auto app installs)..."
    ...
    foreach ($task in $tasksToDisable) { Disable-ScheduledTask -TaskName $task -ErrorAction SilentlyContinue | Out-Null }

    Write-Section "PRIVACY & UI TWEAKS"
    Write-Step "Disabling Task View button on taskbar..."
    ...
    Write-Step "Disabling News and Interests (taskbar feed)..."
    $dshPath = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\Windows Feeds"
    If (!(Test-Path $dshPath)) { New-Item -Path $dshPath -Force | Out-Null }
    Set-ItemProperty -Path $dshPath -Name "EnableFeeds" -Value 0 -Type DWord
    ...
    Write-Step "Disabling Start Menu live tiles data collection..."
    Set-ItemProperty -Path $cdm -Name "SubscribedContent-314559Enabled" -Value 0 -Type DWord
    Set-ItemProperty -Path $cdm -Name "SubscribedContent-314563Enabled" -Value 0 -Type DWord

    Write-Step "Disabling lock screen ads and Spotlight..."
    ...
    Write-Step "Disabling People button on taskbar..."
    $peoplePath = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced\People"
    If (!(Test-Path $peoplePath)) { New-Item -Path $peoplePath -Force | Out-Null }
    Set-ItemProperty -Path $peoplePath -Name "PeopleBand" -Value 0 -Type DWord

    Write-Step "Disabling Meet Now button on taskbar..."
    $meetPath = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Explorer"
    If (!(Test-Path $meetPath)) { New-Item -Path $meetPath -Force | Out-Null }
    Set-ItemProperty -Path $meetPath -Name "HideSCAMeetNow" -Value 1 -Type DWord
...
function Invoke-Debloat11 {

    Write-Section "REMOVING BLOATWARE APPS"
    $appsToRemove = @(
        "Microsoft.3DBuilder","Microsoft.BingFinance","Microsoft.BingFoodAndDrink","Microsoft.BingHealthAndFitness",
        "Microsoft.BingMaps","Microsoft.BingNews","Microsoft.BingSports","Microsoft.BingTranslator","Microsoft.BingTravel",
        "Microsoft.BingWeather","Microsoft.GetHelp","Microsoft.Getstarted","Microsoft.Messaging","Microsoft.Microsoft3DViewer",
        ...
        "Microsoft.Office.OneNote","Microsoft.Office.Sway","Microsoft.OneConnect","Microsoft.OutlookForWindows",
        "Microsoft.People","Microsoft.PowerAutomateDesktop","Microsoft.Print3D","Microsoft.SkypeApp",
        "Microsoft.StorePurchaseApp","Microsoft.Todos","Microsoft.Wallet","Microsoft.WebMediaExtensions",
        "Microsoft.WebpImageExtension","Microsoft.WindowsFeedbackHub","Microsoft.WindowsMaps","Microsoft.WindowsPhone",
        "Microsoft.WindowsSoundRecorder","Microsoft.YourPhone","Microsoft.ZuneVideo",
        "Microsoft.GamingApp","Microsoft.XboxApp","Microsoft.XboxGameCallableUI","Microsoft.XboxGameOverlay",
        "Microsoft.XboxGamingOverlay","Microsoft.XboxIdentityProvider","Microsoft.XboxSpeechToTextOverlay","Microsoft.Xbox.TCUI",
        "MicrosoftTeams","MSTeams","Microsoft.Teams",
        "Microsoft.Windows.Ai.Copilot.Provider","Microsoft.Copilot","MicrosoftWindows.Client.AI",
        ...
        "king.com.CandyCrushSaga","king.com.CandyCrushSodaSaga","LinkedInforWindows","MarchofEmpires","Netflix",
        "PandoraMediaInc","Plex","PricelinePartnerNetwork","Shazam","Spotify","SpotifyAB.SpotifyMusic","TheNewYorkTimes",
        "TikTok","TuneIn","Twitter","Wunderlist"
    )
    foreach ($app in $appsToRemove) { Remove-AppxSafe $app }
...
function Show-SubMenu($osName) {
    Clear-Host
    Write-Host ""
    Write-Host " ############################################" -ForegroundColor Cyan
    Write-Host " #        $osName DEBLOAT TOOL" -ForegroundColor Cyan
    Write-Host " ############################################" -ForegroundColor Cyan
    Write-Host ""
    Write-Host " [1] Full Debloat (includes Remove Edge)" -ForegroundColor Green
    Write-Host "     Removes bloatware/OneDrive/trackers," -ForegroundColor DarkGray
    Write-Host "     disables telemetry, and removes Edge." -ForegroundColor DarkGray
    Write-Host ""
...
function Run-Full($osName, $debloatFn) {
    Write-Host ""
    Write-Host " Starting Full Debloat for $osName (Edge will be removed too)..." -ForegroundColor Cyan
    Write-Host " Make sure Chrome or Firefox is already installed!" -ForegroundColor Red
...
function Run-SubMenu($osName, $debloatFn) {
    do {
        Show-SubMenu $osName
        ...
        switch ($choice) {
            "1" { Run-Full $osName $debloatFn }
            "0" { return }
            "Q" { Write-Host ""; Write-Host " Exiting. Goodbye!" -ForegroundColor DarkGray; Exit }
            default {
                Write-Host ""
...
do {
    ...
    switch ($choice) {
        "1" { Run-SubMenu "WINDOWS 10" ${function:Invoke-Debloat10} }
        "2" { Run-SubMenu "WINDOWS 11" ${function:Invoke-Debloat11} }
        "Q" { Write-Host ""; Write-Host " Exiting. Goodbye!" -ForegroundColor DarkGray; Exit }
        default {
            Write-Host ""
            Write-Host " Invalid option. Try again..." -ForegroundColor Red
